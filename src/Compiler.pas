...
begin
  ...
  StopOptimization;
  optimization.Optimizer.StartOptimization(TokenAt(tokenIndex).SourceLocation);
...
procedure ResetOpty;
begin
  optimization.Optimizer.ResetOpty;
end;
      ...
      begin
        ...
        for i := 1 to NumParams do
          ...
          begin
            ...
            if (Identifier.Param[i].AllocElementType = TDataType.PROCVARTOK) then
            begin

              //  writeln(Identifier.Name,',', IdentifierAt(GetIdentIndex('@FN' + IntToHex(Identifier.Param[i].NumAllocElements shr 16, 4))].NumParams,',',Param[i].AllocElementType,' | ', Identifier.Param[i].DataType,',', Param[i].AllocElementType,',',IdentifierAt(GetIdentIndex('@FN' + IntToHex(Param[i).NumAllocElements shr 16, 4))].NumParams);

              case Param[i].AllocElementType of

                TDataType.PROCEDURETOK, TDataType.FUNCTIONTOK:
                  yes := IdentifierAt(GetIdentIndex('@FN' + IntToHex(Identifier.Param[i].NumAllocElements shr 16, 4))).NumParams = IdentifierAt(GetIdentIndex(Param[i].Name)).NumParams;

                TDataType.PROCVARTOK:
                  yes := (IdentifierAt(GetIdentIndex('@FN' + IntToHex(Identifier.Param[i].NumAllocElements shr 16, 4))).NumParams) = (IdentifierAt(GetIdentIndex('@FN' + IntToHex(Param[i].NumAllocElements shr 16, 4))).NumParams);
...
{
writeln('_C: ', Identifier.Name);

     writeln (Identifier.Name,',',IdentIndex);
     writeln (Identifier.Param[i].DataType,',', Param[i].DataType);
     writeln (Identifier.Param[i].AllocElementType ,',', Param[i].AllocElementType);
...
writeln('_A: ', Identifier.Name);
     ...
     writeln (Identifier.Param[i].NumAllocElements,',', Param[i].NumAllocElements);
            ...
            if (Identifier.Param[i].DataType in IntegerTypes) and (Param[i].DataType in IntegerTypes) then
            begin

              if Identifier.Param[i].DataType in UnsignedOrdinalTypes then
              begin
            ...
            begin
{
writeln('_D: ', Identifier.Name);

     writeln (Identifier.Name,',',IdentIndex, ' - ',Identifier.NumParams,',', NumParams);
     writeln (Identifier.Param[i].DataType,',', Param[i].DataType);
     writeln (Identifier.Param[i].AllocElementType ,',', Param[i].AllocElementType);
     writeln (Identifier.Param[i].NumAllocElements,',', Param[i].NumAllocElements);
}
...
writeln('_B: ', Identifier.Name);
     ...
     writeln (Identifier.Param[i].NumAllocElements,',', Param[i].NumAllocElements);
        ...
        best[k].IdentIndex := Identifier.IdentifierIndex;
        best[k].hit := hits;
        best[k].b := Identifier.BlockIndex;
        ...
        for k := 0 to High(l) - 1 do
          ...
          begin
            ...
            begin
              ...
              if (Identifier.Param[m].DataType = l[k].Param[m].DataType) and
                (Identifier.Param[m].AllocElementType = TDataType.PROCVARTOK) and
                (l[k].Param[m].AllocElementType = TDataType.PROCVARTOK) and
                (Identifier.Param[m].NumAllocElements shr 16 <> l[k].Param[m].NumAllocElements shr 16) then
              begin
...
procedure a65(const code: TCode65; const Value: Int64 = 0; const Kind: TTokenKind = TTokenKind.CONSTTOK; const Size: Byte = 4; const IdentIndex: TIdentifierIndex = 0);
var
  svar: String;
begin
  ...
  case code of
    ...
    TCode65.movaBX_Value: begin
      ...
      begin          // @label
        ...
        if IdentifierAt(IdentIndex).IdType = TDataType.ARRAYTOK then
...
function InfoAboutSize(const Size: Byte): String;
begin
...
begin
  ...
  Result := (IdentIndex > 0) and
    ...
    (((IdentifierAt(IdentIndex).Value >= 0) and (IdentifierAt(IdentIndex).Value < 256)) or

    ((abs(IdentifierAt(IdentIndex).Value) and $FF = 0) and (Byte((abs(IdentifierAt(IdentIndex).Value) shr 24) and $7F) in [1..127])));
end;
...
procedure Push(Value: Int64; IndirectionLevel: TIndirectionLevel; Size: Byte; IdentIndex: TIdentifierIndex = 0; par: Byte = 0);
var
  Kind: TTokenKind;
  NumAllocElements: Cardinal;
  IdentTemp: TIdentifierIndex;
  Page: Smallint;
  ...
  case IndirectionLevel of
    ...
    begin
      ...
      begin
        ...
        if (IdentifierAt(IdentIndex).DataType = TDataType.POINTERTOK) and not (IdentifierAt(IdentIndex).AllocElementType in [TDataType.UNTYPETOK, TDataType.PROCVARTOK]) and
...
procedure GenerateAssignment(IndirectionLevel: Byte; Size: Byte; IdentIndex: TIdentIndex; Param: String = ''; ParamY: String = '');
var
  NumAllocElements: Cardinal;
  IdentTemp: Integer;
  Page: Smallint;
...
function SafeCompileConstExpression(var i: TTokenIndex; out ConstVal: Int64; out ValType: TDataType; VarType: TDataType; Err: Boolean = False; War: Boolean = True): Boolean;
var
  j: Integer;
begin
...
function CompileArrayIndex(i: TTokenIndex; IdentIndex: TIdentIndex; out VarType: TDataType): TTokenIndex;
var
  ConstVal: Int64;
      ...
      begin
        ...
        begin
          ...
          if IdentifierAt(IdentIndex).Kind in
            [TTokenKind.PROCEDURETOK, TTokenKind.FUNCTIONTOK, TTokenKind.CONSTRUCTORTOK, TTokenKind.DESTRUCTORTOK] then
          begin
              ...
              // !!! STRIPED !!! NOT ALLOWED
                ...
                begin
                  ...
                  begin
                    ...
                    asm65(#9'ldy #$00');
                    asm65(#9'lda (:bp2),y');
                    asm65(#9'add #' + HexByte(IdentTemp and $ffff));
                    asm65(#9'sta' + StackVariable0);
                    asm65(#9'iny');
                    asm65(#9'lda (:bp2),y');
                    asm65(#9'adc #$00');
                    ...
                    //    WRITELN(IdentifierAt(IdentIndex).Name,',',TokenAt(i + 3).Name,',',  TDataType(IdentTemp shr 16) ,',',IdentifierAt(IdentIndex).PassMethod);
                ...
                begin
                  ...
                  // writeln(IdentifierAt(IdentIndex).Name,':',IdentifierAt(IdentIndex).DataType,',',IdentifierAt(IdentIndex).AllocElementType,',',IdentifierAt(IdentIndex).NumAllocElements,',',IdentifierAt(IdentIndex).PassMethod,',',IdentifierAt(IdentIndex).idType );
...
procedure CompileActualParameters(var i: TTokenIndex; IdentIndex: Integer; ProcVarIndex: Integer = 0);
var
 ...
 writeln(IdentifierAt(IdentIndex).Param[NumActualParams].PassMethod,',', IdentifierAt(IdentTemp).PassMethod);
        ...
        begin
          ...
          if (ActualParamType in IntegerTypes) and
            (IdentifierAt(IdentIndex).Param[NumActualParams].DataType in RealTypes) then
          begin
          ...
          if (TokenAt(i).Kind = IDENTTOK) and
            (IdentifierAt(IdentIndex).Param[NumActualParams].DataType = TDataType.ENUMTOK) then
          begin
            IdentTemp := GetIdentIndex(TokenAt(i).Name);
 ...
 writeln(IdentifierAt(IdentIndex).Param[NumActualParams].PassMethod,',', IdentifierAt(IdentTemp).PassMethod);
          ...
          begin
            ...
            begin
              ...
              if (IdentifierAt(IdentIndex).Param[NumActualParams].DataType in [TDataType.POINTERTOK, TDataType.STRINGPOINTERTOK]) and
...

{
 if (IdentifierAt(IdentIndex).isExternal) and (IdentifierAt(IdentIndex).Libraries > 0) then begin
  ...
  if RCLIBRARY and
    IdentifierAt(IdentIndex).isExternal and
    (IdentifierAt(IdentIndex).Libraries > 0) and
        ...
        begin      // only ONE parameter SIZE = 1
          ...
          case ActualParamType of
            ...
            begin
              ...
              Error(i, TMessage.Create(TErrorCode.Unassigned, 'Unassigned: {0}', InfoAboutDataType(ActualParamType)));
          end;
    end;
  ...
  begin
    ...
    if (IdentifierAt(IdentIndex).BlockIndex > 1) and (IdentifierAt(IdentIndex).BlockIndex <> BlockStackTopBlockIndex) then
      // issue #102 fixed
...

{
  ...
  if (IdentifierAt(IdentIndex).Kind = TTokenKind.FUNCTIONTOK) and
    (IdentifierAt(IdentIndex).isStdCall = False) and
...
function CompileFactor(i: TTokenIndex; out isZero: Boolean; out ValType: TDataType; VarType: TDataType = TDataType.INTEGERTOK): TTokenIndex;
var
  IdentTemp, IdentIndex: TIdentIndex;
  OldCodeSize: Integer;
  j: TTokenIndex;
              ...
              begin
                ...
                if IdentTemp = 0 then
                  ...
                  begin
                    ...
                    if IdentifierAt(IdentIndex).NumParams <> NumParam then
              ...
              begin
                ...
                if (TokenAt(i + 1).Kind = TTokenKind.DEREFERENCETOK) then
                  ...
                  begin
                    ...
                    if (IdentifierAt(IdentIndex).DataType = TDataType.STRINGPOINTERTOK) and (IdentifierAt(IdentIndex).NumAllocElements = 0) then
                    ...
                    begin
                      ...
                      Push(0, ASPOINTERTOARRAYORIGIN2, GetDataSize(ValType), IdentIndex);

                    end
                    else
                      if (ValType = TDataType.UNTYPETOK) and
                        (IdentifierAt(IdentIndex).DataType = TDataType.POINTERTOK) then
                      begin
                          ...
                          begin
                            ...
                            begin              // record^.label[x]
                              ...
                              if ValType = TDataType.STRINGPOINTERTOK then
                                Push(IdentifierAt(IdentIndex).Value, ASPOINTERTORECORD, GetDataSize(ValType), IdentIndex, IdentTemp and $ffff)
                              // record^.string
                              else
                                Push(IdentifierAt(IdentIndex).Value, ASPOINTERTOPOINTER, GetDataSize(ValType), IdentIndex, IdentTemp and $ffff);
                            // record_lebel.field^
...
function CompileSimpleExpression(i: TTokenIndex; out ValType: TDataType; VarType: TDataType): TTokenIndex;
var
  j, k: TTokenIndex;
  ConstVal: Int64;
  RightValType: TDataType;
...
var
  j, k: TTokenIndex;
  IdentIndex, IdentTemp: TIdentIndex;
  NumActualParams, NumCharacters, IfLocalCnt, CaseLocalCnt, NumCaseStatements, vlen: Integer;
  OldPass: TPass;
  OldCodeSize: Integer;
  Param: TParamList;
  NumParam: Integer;
      ...
      if IdentIndex > 0 then
        ...
        case IdentifierAt(IdentIndex).Kind of
          ...
          begin
            ...
            if (TokenAt(i + 1).Kind = OPARTOK) and
              (IdentifierAt(IdentIndex).DataType = TDataType.POINTERTOK) and
            ...
            begin
              ...
              //  writeln(VarType,',',ExpressionType,' | ', IdentifierAt(IdentIndex).DataType,',',IdentifierAt(IdentIndex).AllocElementType,',',IdentifierAt(IdentIndex).NumAllocElements);
                ...
                if GetDataSize(VarType) <> Elements(IdentIndex) * GetDataSize(IdentifierAt(IdentIndex).AllocElementType) then
                  ...
                  if Elements(GetIdentIndex(TokenAt(j + 2).Name)) = 0 then
                    ...
                    Error(j + 2, 'Illegal type conversion: "Array[0..' +
                      IntToStr(Elements(GetIdentIndex(TokenAt(j + 2).Name)) - 1) + '] Of ' + InfoAboutDataType(VarType) +
                      '" to "' + IdentifierAt(IdentIndex).Name + '"');
              ...
              begin        // (pointer)
                ...
                begin
                  ...
                  if TokenAt(i + 2).Kind = TTokenKind.OBRACKETTOK then     // pp^[index] :=
                  begin
        ...
        writeln('d');
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        // writeln('> ',IdentifierAt(IdentIndex).Name,',',vartype,',',IdentifierAt(IdentIndex).DataType,',',IdentifierAt(IdentIndex).AllocElementType,',',IdentifierAt(IdentIndex).NumAllocElements,',',IdentifierAt(IdentIndex).idType,' | ', TokenAt(i + 2).Kind,',',TokenAt(i + 3).Kind);
                  ...
                  begin
                    ...
                    begin
                      ...
                      if (IdentifierAt(IdentIndex).AllocElementType = TDataType.UNTYPETOK) or
                        ...
                        (IdentifierAt(IdentIndex).IsObjectVariable and (IdentifierAt(IdentIndex).NestedDataType = TDataType.POINTERTOK) and (IdentifierAt(IdentIndex).NestedNumAllocElements = 0)) then
            ...
            begin
              ...
              begin                // Usual assignment
                ...
                if (TokenAt(i).Kind = TTokenKind.DEREFERENCETOK) and
                  (VarType = TDataType.POINTERTOK) and
                ...
                if VarType <> ExpressionType then
                  if (ExpressionType = TDataType.POINTERTOK) and (TokenAt(k).Kind = TTokenKind.IDENTTOK) then
                    ...
                    begin
                      ...
                      IdentTemp := GetIdentIndex('@FN' + IntToHex(IdentifierAt(GetIdentIndex(TokenAt(k).Name)).NumAllocElements_, 4));
          ...
          Error(k, IncompatibleTypesArray, IdentTemp, ExpressionType )
                      ...
                      // CheckCommonType(i + 1, VarType, TTokenKind.RECORDTOK)
                        ...
                        begin
                          ...
                          if (IdentifierAt(IdentTemp).NumAllocElements > 0) and (IdentifierAt(IdentIndex).NumAllocElements > 0) then
                    ...
                    begin
                      ...
                      if (IdentTemp > 0) and
                        ((IdentifierAt(IdentTemp).Kind = TTokenKind.ENUMTOK) or (IdentifierAt(IdentTemp).DataType = TDataType.ENUMTOK)) then
                      begin
                ...
                if (IdentifierAt(IdentIndex).PassMethod <> TParameterPassingMethod.VARPASSING) and
                  ...
                  (IdentifierAt(IdentIndex).DataType = TDataType.POINTERTOK) and
                  (IdentifierAt(IdentIndex).NumAllocElements = 0) and
                  (ExpressionType <> TDataType.POINTERTOK) then
                begin

                  if (IdentifierAt(IdentIndex).AllocElementType in OrdinalTypes) and (ExpressionType in OrdinalTypes) then
                    ...
                    begin
                      ...
                      if (ExpressionType in [TDataType.PCHARTOK, TDataType.STRINGPOINTERTOK]) and
                        ...
                        Error(i + 1, 'Incompatible types: got "' +
                          InfoAboutDataType(ExpressionType) +
                ...
                begin
                  ...
                  begin
                    ...
                    if IdentifierAt(IdentTemp).Kind = TTokenKind.FUNCTIONTOK then
                      yes := IdentifierAt(IdentIndex).NumAllocElements <> IdentifierAt(GetIdentResult(IdentifierAt(IdentTemp).ProcAsBlockIndex)).NumAllocElements
                    else
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          if IndirectionLevel in [ASPOINTERTOARRAYORIGIN, ASPOINTERTOARRAYORIGIN2] then
                          begin

                            if (IdentifierAt(IdentIndex).AllocElementType = TDataType.RECORDTOK) and
                              (IdentifierAt(IdentIndex).NumAllocElements_ > 0) and
                              (IdentifierAt(IdentTemp).AllocElementType = TDataType.RECORDTOK) and
                          ...
                          begin
                            ...
                            begin
                              ...
                              if (IdentifierAt(IdentTemp).AllocElementType = TDataType.RECORDTOK) and
                                (IdentifierAt(IdentTemp).NumAllocElements_ > 0) then
                              begin
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    asm65(#9'@move ":bp2" ' + Name + GetLocalName(IdentIndex, 'adr.') + ' #' + IntToStr(RecordSize(IdentIndex)));
...
{
          if (TokenAt(k).Kind <> TTokenKind.ADDRESSTOK) and (IndirectionLevel in [ASPOINTER, ASPOINTERTOPOINTER]) then begin

           if (DEREFERENCE = false) and (IdentifierAt(IdentIndex).NumAllocElements = 0) and (TokenAt(k).Kind = TTokenKind.STRINGLITERALTOK) then
             Error(i, 'Incompatible types: got "' + InfoAboutToken(TokenAt(k).Kind) + '" expected "^' + IdentifierAt(IdentIndex).Name + '"');

           if (DEREFERENCE = false) and (IdentifierAt(IdentIndex).NumAllocElements = 0) and (TokenAt(k).Kind = TTokenKind.IDENTTOK) and (IdentifierAt(GetIdentIndex(TokenAt(k).Name)).NumAllocElements > 0) then
             Error(i, 'Incompatible types: got "' + InfoAboutToken(IdentifierAt(GetIdentIndex(TokenAt(k).Name)).Kind) + '" expected "' + IdentifierAt(IdentIndex).Name + '"');

           if DEREFERENCE and (IdentifierAt(IdentIndex).NumAllocElements = 0) and (TokenAt(k).Kind = TTokenKind.IDENTTOK) and (TokenAt(k + 1).Kind <> TTokenKind.DEREFERENCETOK) and (IdentifierAt(GetIdentIndex(TokenAt(k).Name)).NumAllocElements = 0) then
             Error(i, 'Incompatible types: got "^' + InfoAboutToken(IdentifierAt(GetIdentIndex(TokenAt(k).Name)).Kind) + '" expected "' + InfoAboutToken(IdentifierAt(GetIdentIndex(TokenAt(k).Name)).Kind) + '"');

          end;
}

                    if (TokenAt(k).Kind <> TTokenKind.ADDRESSTOK) and
                      (IndirectionLevel in [ASPOINTERTOARRAYORIGIN, ASPOINTERTOARRAYORIGIN2]) and
                      (IdentifierAt(IdentIndex).AllocElementType = TDataType.STRINGPOINTERTOK) then
                    begin
                              ...
                              begin
{
          ...
          TTokenKind.PROCEDURETOK, TTokenKind.FUNCTIONTOK,
          TTokenKind.CONSTRUCTORTOK, TTokenKind.DESTRUCTORTOK:    // Procedure, Function (without assignment) call
          begin
      ...
      begin
        ...
        case IdentifierAt(IdentTemp).Kind of
          ...
          FUNCTIONTOK: begin
            ...
            if SelectorType = TDataType.UNTYPETOK then
              SelectorType := IdentifierAt(GetIdentResult(IdentifierAt(IdentTemp).ProcAsBlockIndex)).DataType;
    ...
    begin
      ...
      begin
        ...
        optimization.Optimizer.SetOptyBP2('');
    ...
    begin
      ...
      Inc(CodeSize);    // !!! so that nested WHILE loops can work
                ...
                begin
                  ...
                  begin
                    ...
                    DefineIdent(j, '@FORTMP_' + IntToHex(CodeSize, 4), TTokenKind.CONSTTOK, VarType,
                      IdentifierAt(IdentIndex).NumAllocElements, IdentifierAt(IdentIndex).AllocElementType, ConstVal, TokenAt(j).GetDataType);
                  ...
                  begin
                    ...
                    begin
                      ...
                      optimization.Optimizer.SetOptyY('');
                      ...
                      case GetDataSize(ActualParamType) of
                        ...
                        2: begin
                          asm65(#9'ldy #' + IntToHex(Byte(ConstVal)));
                          asm65(#9'sty ' + GetLocalName(IdentIndex));
                          asm65(#9'ldy #' + IntToHex(Byte(ConstVal shr 8)));
                          asm65(#9'sty ' + GetLocalName(IdentIndex) + '+1');
                        end;
                        ...
                        4: begin
                          ...
                          asm65(#9'ldy #' + IntToHex(Byte(ConstVal shr 8)));
                          asm65(#9'sty ' + GetLocalName(IdentIndex) + '+1');
                          asm65(#9'ldy #' + IntToHex(Byte(ConstVal shr 16)));
                          asm65(#9'sty ' + GetLocalName(IdentIndex) + '+2');
                          asm65(#9'ldy #' + IntToHex(Byte(ConstVal shr 24)));
                          asm65(#9'sty ' + GetLocalName(IdentIndex) + '+3');
                        end;
      ...
      begin
        ...
        begin
          ...
          case TokenAt(i + 1).Kind of
            ...
            begin
              asm65(#9'ldy #$00');
              asm65(#9'mva:rne CODEORIGIN+' + HexWord(Word(TokenAt(i + 1).StrAddress - CODEORIGIN + 1)) + ',y @buf,y+');

              if yes then
              begin                 // WRITELN

                asm65(#9'lda #eol');
                asm65(#9'ldy CODEORIGIN+' + HexWord(Word(TokenAt(i + 1).StrAddress - CODEORIGIN)));
                asm65(#9'sta @buf,y');
              ...
              begin                // WRITE
                ...
                asm65(#9'lda CODEORIGIN+' + HexWord(Word(TokenAt(i + 1).StrAddress - CODEORIGIN)));
          ...
          repeat
            ...
            case TokenAt(i + 1).Kind of
              ...
              begin
                ...
                if (ExpressionType in IntegerTypes) then    // Integer argument
        ...
        begin
          ...
          begin
            ...
            begin      // Alloc Element Type
              ...
              if pos('mw? ' + TokenAt(i).Name, optimization.Optimizer.GetOptyBP2) > 0 then
                optimization.Optimizer.SetOptyBP2('');
            end;
...
var
  ...
  begin
    ...
    begin
      ...
      if brackets then
        Result := #9'= [DATAORIGIN+' + HexWord(Word(IdentifierAt(IdentIndex).Value - DATAORIGIN)) + ']'
      else
        Result := #9'= DATAORIGIN+' + HexWord(Word(IdentifierAt(IdentIndex).Value - DATAORIGIN));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          asm65('');
          asm65(#9'lmb #' + HexByte(IdentifierAt(IdentTemp).Value + 1));
          asm65('');
...
procedure FormalParameterList(var i: TTokenIndex; var NumParams: Integer; var Param: TParamList; out Status: Word; IsNestedFunction: Boolean; out NestedFunctionResultType: TDataType; out NestedFunctionNumAllocElements: Cardinal; out NestedFunctionAllocElementType: TDataType);
var
...
procedure CompileRecordDeclaration(i: TTokenIndex; var VarOfSameType: TVariableList; var tmpVarDataSize: Integer; var ConstVal: Int64; VarOfSameTypeIndex: Integer; VarType, AllocElementType: TDataType; NumAllocElements: Cardinal; isAbsolute: Boolean; var idx: Integer);
var
    ...
    begin
      ...
      begin
        ...
        if isAbsolute = false then
          Error(i, 'Type ' + GetTypeAtIndex(NumAllocElements).Field[0].Name + ' requires absolute address on PAGE ' + HexByte(Byte(GetTypeAtIndex(NumAllocElements).Page - 1)));

        if (ConstVal and $FF00 <> (GetTypeAtIndex(NumAllocElements).Page - 1) shl 8) then
          Error(i, GetTypeAtIndex(NumAllocElements).Field[0].Name + ' requires PAGE ' + HexByte(Byte(GetTypeAtIndex(NumAllocElements).Page - 1)) + ', got ' + HexByte(Byte(ConstVal shr 8)));
...
function CompileBlock(i: TTokenIndex; BlockIdentIndex: Integer; NumParams: Integer; IsFunction: Boolean; FunctionResultType: TDataType; FunctionNumAllocElements: Cardinal = 0; FunctionAllocElementType: TDataType = TDataType.UNTYPETOK): Integer;
var
  VarOfSameType: TVariableList;
  VarPassMethod: TParameterPassingMethod;
  Param: TParamList;
  j: TTokenIndex;
  idx, NumVarOfSameType, VarOfSameTypeIndex, tmpVarDataSize, ParamIndex, ForwardIdentIndex, IdentIndex, external_libr: Integer;
  ...
  {pack,} IsNestedFunction, isAbsolute, isExternal, isForward, isVolatile, isStriped, isAsm, isReg, isInt, isInl, isOvr: Boolean;
  VarType: TDataType;
  VarRegister: Byte;
  NestedFunctionResultType, ConstValType, AllocElementType, ActualParamType, NestedFunctionAllocElementType, NestedDataType, NestedAllocElementType, IdType: TDataType;
  Tmp, TmpResult: Word;
...
begin
  ...
  begin
    ...
    DefineIdent(i, GetTypeAtIndex(IdentifierAt(BlockIdentIndex).ObjectIndex).Field[0].Name, TTokenKind.VARTOK, TDataType.WORDTOK, 0, TDataType.UNTYPETOK, 0);
    IdentifierAt(NumIdent).PassMethod := TParameterPassingMethod.VARPASSING;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DefineIdent(i, Param[ParamIndex].Name, TTokenKind.VARTOK, Param[ParamIndex].DataType, Param[ParamIndex].NumAllocElements, Param[ParamIndex].AllocElementType, 0);
...

{
  ...
  begin
    ...
    begin    // at most after the PROGRAM
      ...
      repeat
        ...
        if (TokenAt(i).Name <> 'SYSTEM') and not ActiveSourceFile.IsAllowedUnitName(TokenAt(i).Name) then
        begin
          if not ActiveSourceFile.AddAllowedUnitName(TokenAt(i).Name) then
          ...
          begin
            ...
            begin
              ...
              if TokenAt(j).StrLength > 255 then
                DefineIdent(i + 1, TokenAt(i + 1).Name, TTokenKind.CONSTTOK, TDataType.POINTERTOK, 0, TDataType.CHARTOK, ConstVal + CODEORIGIN, TDataType.PCHARTOK)
              else
                DefineIdent(i + 1, TokenAt(i + 1).Name, TTokenKind.CONSTTOK, ConstValType, TokenAt(j).StrLength, TDataType.CHARTOK, ConstVal + CODEORIGIN, TokenAt(j).GetDataType);
              ...
              if (ConstValType in Pointers) then
                ...
                DefineIdent(i + 1, TokenAt(i + 1).Name, TTokenKind.CONSTTOK, ConstValType, 0, TDataType.UNTYPETOK, ConstVal, TokenAt(j).GetDataType);
            ...
            begin
              ...
              begin
                ...
                if TokenAt(i + 3).Kind = TTokenKind.PCHARTOK then
                  DefineIdent(i + 1, TokenAt(i + 1).Name, TTokenKind.CONSTTOK, TDataType.POINTERTOK, 0, TDataType.CHARTOK, ConstVal + CODEORIGIN + 1, TDataType.PCHARTOK)
                else
                  DefineIdent(i + 1, TokenAt(i + 1).Name, TTokenKind.CONSTTOK, ConstValType, TokenAt(j).StrLength, TDataType.CHARTOK, ConstVal + CODEORIGIN, TokenAt(j).GetDataType);
                ...
                begin
                  ...
                  DefineIdent(i + 1, TokenAt(i + 1).Name, TTokenKind.CONSTTOK, VarType, 0, TDataType.UNTYPETOK, ConstVal, TokenAt(j).GetDataType);
                end;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            DefineIdent(i + 1, TokenAt(i + 1).Name, TTokenKind.TYPETOK, VarType, NumAllocElements, AllocElementType, 0, TokenAt(i + 3).GetDataType);
    ...
    begin
      ...
      repeat
        ...
        begin
          ...
          begin
            DefineIdent(i, VarOfSameType[VarOfSameTypeIndex].Name, TTokenKind.VARTOK, VarType, NumAllocElements, AllocElementType, Ord(isAbsolute) * ConstVal, IdType);
                  ...
                  begin
                    ...
                    begin
                      ...
                      if not ((IdentifierAt(IdentIndex).DataType in Pointers) and (IdentifierAt(IdentIndex).NumAllocElements > 0)) then
                        ...
                        SaveToDataSegment(idx, IdentifierAt(IdentIndex).Value - CODEORIGIN - CODEORIGIN_BASE, TDataType.CODEORIGINOFFSET);
                      ...
                      begin      // array [] of type = ( )
                        ...
                        begin    // = 'string'
                          ...
                          i := ReadDataArray(i, idx, IdentifierAt(NumIdent).AllocElementType,
                            IdentifierAt(NumIdent).NumAllocElements or IdentifierAt(NumIdent).NumAllocElements_ shl 16, False,
      ...
      begin
        ...
        if ForwardIdentIndex <> 0 then
          if not IdentifierAt(ForwardIdentIndex).isUnresolvedForward or
            ...
            ((TokenAt(i).Kind = TTokenKind.PROCEDURETOK) and (IdentifierAt(ForwardIdentIndex).Kind <> TTokenKind.PROCEDURETOK)) or
            ...
            ((TokenAt(i).Kind = TTokenKind.FUNCTIONTOK) and (IdentifierAt(ForwardIdentIndex).Kind <> TTokenKind.FUNCTIONTOK)) then
...
//      GenerateAssignment(IdentifierAt(IdentIndex).Value, ASPOINTER, GetDataSize(TDataType.POINTERTOK), IdentIndex);
  ...
  if IdentifierAt(BlockIdentIndex).Kind in
    [TTokenKind.PROCEDURETOK, TTokenKind.FUNCTIONTOK, TTokenKind.CONSTRUCTORTOK, TTokenKind.DESTRUCTORTOK] then
  begin
...
if LIBRARY_USE = FALSE then begin
  ...
  begin
    ...
    yes := False;
    for IdentIndex := 1 to NumIdent do
      if (resArray[i].resName = IdentifierAt(IdentIndex).Name) and (IdentifierAt(IdentIndex).BlockIndex = 1) then
      begin
  ...
  optimization.Optimizer.Finalize;
...
begin
  ...
  optimization.Optimizer := Optimizer.CreateDummyOptimizer;
  ...
  optimization.Optimizer.Initialize(target, AsmBlock, AsmBlockIndex, TFileWriter.Create(OutFile), OptFile);
