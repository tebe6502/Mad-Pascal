...
Type
  ...
  TBFBlock     = array[0..1] of cardinal;     { BlowFish }
...
procedure Create(Key : TBlowFishKey; KeySize : word); overload;
var
  I     : word;
  J     : word;
  K     : word;
...
begin
  If (KeyPhrase='') then halt;

  KLen:=Length(KeyPhrase);
  If KLen>56 then KLen:=56;
...
begin
  ...
  While Count > 0 do begin
    ...
    If (FBufPos > 0) then
      begin
      mvSize:=FBufPos;
      If MvSize > count then mvsize:=Count;

      Move(P[0], Result[FPos], MVSize);
