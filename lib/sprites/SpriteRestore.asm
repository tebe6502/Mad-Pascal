
//---------------------------------------------------------------------
//	SPRITE RESTORE
//---------------------------------------------------------------------

.local	SpriteRestore

	sta	zp+@zp.hlp0
	sty	zp+@zp.hlp0+1

	ldy	#PlayfieldWidth*0
	mva	CharsBackup+0,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+1,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+2,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+3,x		(zp+@zp.hlp0),y

	ldy	#PlayfieldWidth*1
	mva	CharsBackup+4,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+5,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+6,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+7,x		(zp+@zp.hlp0),y

	ldy	#PlayfieldWidth*2
	mva	CharsBackup+8,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+9,x		(zp+@zp.hlp0),y+
	mva	CharsBackup+10,x	(zp+@zp.hlp0),y+
	mva	CharsBackup+11,x	(zp+@zp.hlp0),y

	ldy	#PlayfieldWidth*3
	mva	CharsBackup+12,x	(zp+@zp.hlp0),y+
	mva	CharsBackup+13,x	(zp+@zp.hlp0),y+
	mva	CharsBackup+14,x	(zp+@zp.hlp0),y+
	mva	CharsBackup+15,x	(zp+@zp.hlp0),y

	rts
.endl
