...
var
    i, ii, col : byte;
    palete_r   : array [0..255] of byte;
    palete_g   : array [0..255] of byte;
    palete_b   : array [0..255] of byte;
...
begin
    for i := 15 downto 0 do begin
        ...
        for ii := 15 downto 0 do begin
            v := (ii / 15);
            col := i * 16 + ii;
            palete_r[col] := round(r * v);
            palete_g[col] := round(g * v);
            palete_b[col] := round(b * v);

            NeoSetPalette(col, palete_r[col], palete_g[col], palete_b[col]);
        end;
    end;
end;
...
begin
    ...
    repeat
        NeoWaitForVblank;
        for i := 0 to 255 do
            NeoSetPalette(i + col, palete_r[i], palete_g[i], palete_b[i]);
        inc(col);
    until false;
end.