...
var
  d: word;
  ss,nn,a,b,c,lin,rlin,x,y,pus: byte;
...
begin
  ...
  case ss of
    0: begin scr[j,i]:=#32; scr[j,i+1]:=#32 end; // '  '
    1: begin scr[j,i]:=#123; scr[j,i+1]:=#125 end; // '[]'
    2: begin scr[j,i]:=#95; scr[j,i+1]:=#95 end; // '  '*
  end;
    
  gotoxy(i, j);
  write(scr[j,i]);write(scr[j,i+1]);
...
begin
  ...
  repeat
    ...
    nn:=1+random(18);
    x:=6;y:=20; fig(x,y,nn,3); d:=500-(lin*10);
