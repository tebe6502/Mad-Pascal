...
// ---------------------------------------------------------------------------
  ...
  begin
    ...
    for IdentIndex := 1 to NumIdent do
      if (Ident[IdentIndex].Block = ProcAsBlock) and (Ident[IdentIndex].Name = 'RESULT') then
       exit(IdentIndex);
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if (lab <> a) and (pos(UnitName[Ident[IdentIndex].UnitIndex].Name + '.', a) = 1) then
    begin
...
// ----------------------------------------------------------------------------
  ...
  function GetIdentProc(const S: TString; ProcIdentIndex: Integer; Param: TParamList; NumParams: Integer): Integer;
...
//	    ( (Ident[IdentIndex].Param[i].DataType = UNTYPETOK) and (Ident[IdentIndex].Param[i].PassMethod = VARPASSING) and (Param[i].DataType in OrdinalTypes {+ [POINTERTOK]} {IntegerTypes + [CHARTOK]}) )
	   ...
	   then begin
              ...
              if (Ident[IdentIndex].Param[i].AllocElementType = PROCVARTOK) then
              begin
...
writeln('_C: ', Ident[IdentIndex].Name);
     ...
     writeln (Ident[IdentIndex].Param[i].NumAllocElements,',', Param[i].NumAllocElements);
              ...
              if (Ident[IdentIndex].Param[i].DataType = UNTYPETOK) and (Param[i].DataType = POINTERTOK) and
                (Ident[IdentIndex].Param[i].AllocElementType = UNTYPETOK) and (Param[i].AllocElementType <> UNTYPETOK) and (Param[i].NumAllocElements > 0)
...
writeln('_A: ', Ident[IdentIndex].Name);
     ...
     writeln (Ident[IdentIndex].Param[i].NumAllocElements,',', Param[i].NumAllocElements);
		...
		if (Param[i].AllocElementType in [OBJECTTOK, RECORDTOK]) and (Param[i].NumAllocElements_ = 0) then
...
// ----------------------------------------------------------------------------
  ...
  procedure TestIdentProc(x: Integer; const S: TString);
  var
      ...
      for IdentIndex := NumIdent downto 1 do
        ...
        begin
          ...
          for k := 0 to High(l) - 1 do
            if (Ident[IdentIndex].NumParams = l[k].NumParams) and (Ident[IdentIndex].UnitIndex = l[k].u) and (Ident[IdentIndex].Block = l[k].b) then
            begin
              ...
              begin
                if (Ident[IdentIndex].Param[m].DataType <> l[k].Param[m].DataType) or (Ident[IdentIndex].Param[m].AllocElementType <> l[k].Param[m].AllocElementType) then
                begin
                  ok := False;
                  Break;
                end;
                ...
                if (Ident[IdentIndex].Param[m].DataType = l[k].Param[m].DataType) and (Ident[IdentIndex].Param[m].AllocElementType = PROCVARTOK) and
                  (l[k].Param[m].AllocElementType = PROCVARTOK) and (Ident[IdentIndex].Param[m].NumAllocElements shr 16 <> l[k].Param[m].NumAllocElements shr 16) then
                begin
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    begin
      ...
      if (DataSize[ValType] < DataSize[RightValType]) and ((VarType = 0) or (DataSize[RightValType] >= DataSize[VarType])) then
      begin
...
// ----------------------------------------------------------------------------
  ...
  procedure LoadBP2(IdentIndex: Integer; const svar: String);
  var
    lab: String;
  begin
    ...
    begin
      ...
      if Ident[GetIdent(lab)].AllocElementType = RECORDTOK then
      begin
...
{$ENDIF}
    ...
    case IndirectionLevel of
      ...
      begin
{$IFDEF USEOPTFILE}
        asm65('; as Value ' + Hex(Value, 8) + ' (' + IntToStr(Value) + ')');
        asm65;
{$ENDIF}
...
{$ENDIF}
        ...
        case Size of
          ...
          2: begin
            ...
            begin
              ...
              begin
                ...
                asm65(#9'iny');
                asm65(#9'sta :STACKORIGIN,x');
                asm65(#9'sty :STACKORIGIN+STACKWIDTH,x');
              end
              else
              begin
                asm65(#9'mva ' + svar + ' :STACKORIGIN,x');
          ...
          4: begin
            asm65(#9'mva ' + svar + ' :STACKORIGIN,x');
            asm65(#9'mva ' + svar + '+1 :STACKORIGIN+STACKWIDTH,x');
            asm65(#9'mva ' + svar + '+2 :STACKORIGIN+STACKWIDTH*2,x');
            asm65(#9'mva ' + svar + '+3 :STACKORIGIN+STACKWIDTH*3,x');
          end;
              ...
              begin
{
        ...
        asm65(#9'sta '+svara+',y');
          ...
          2: if Ident[IdentIndex].PassMethod = VARPASSING then
            begin
          ...
          4: if Ident[IdentIndex].PassMethod = VARPASSING then
            begin
...
{$ENDIF}
        ...
        begin
          ...
          begin
            ...
            if (IdentTemp > 0) and (Ident[IdentTemp].DataType = POINTERTOK) and
               (Ident[IdentTemp].AllocElementType = RECORDTOK) and (Ident[IdentTemp].NumAllocElements_ > 1) and
...
{$ENDIF}
      ...
      begin
        ...
        a65(__subBX);              // maksymalnie mozemy uzyc :STACKORIGIN-1 lub :STACKORIGIN+1, pomagamy przez DEX/INX
...
// ----------------------------------------------------------------------------
  ...
  end;  //GenerateCaseStatementEpilog
  ...
  begin
    ...
    case IndirectionLevel of
      ...
      begin
        ...
        asm65(#9'@printSTRING #CODEORIGIN+' + Hex(Address - CODEORIGIN, 4));
...
// ----------------------------------------------------------------------------
  ...
  end;  //SafeCompileConstExpression
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if ((Ident[IdentIndex].DataType = POINTERTOK) and (Ident[IdentIndex].IdType = DEREFERENCEARRAYTOK)) then
    begin
    ...
    begin
      ...
      begin
        ExpandParam(WORDTOK, ArrayIndexType);				//      ArrayIndexType := WORDTOK;
      end
      else
        ArrayIndexType := WORDTOK;
...
//        ExpandParam(WORDTOK, ArrayIndexType);
    ...
    begin
      ...
      if (Tok[i + 1].Kind = CBRACKETTOK) and (Tok[i + 2].Kind <> OBRACKETTOK)
    ...
    if {Tok[i + 1].Kind = COMMATOK} yes then
    begin
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    begin
      ...
      if VarPass then
        Error(i + 1, CantAssignValuesToAddress);
    ...
    if (Tok[i + 1].Kind = PCHARTOK) and (Tok[i + 2].Kind = OPARTOK) then
    begin
      ...
      if Tok[j + 2].Kind <> DEREFERENCETOK then
        Error(i + 3, CantAssignValuesToAddress);
...
//	writeln(Ident[IdentIndex].Name,' = ',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].PassMethod );
	    ...
	    if (Tok[i + 2].Kind = DEREFERENCETOK) and (Tok[i + 3].Kind = OBRACKETTOK) and (Ident[IdentIndex].DataType in [POINTERTOK, DEREFERENCEARRAYTOK]) then inc(i);
            ...
            if Ident[IdentIndex].Kind in [PROCEDURETOK, FUNCTIONTOK, CONSTRUCTORTOK, DESTRUCTORTOK] then
            begin
	         ...
	         (Ident[IdentIndex].DataType in Pointers + [DEREFERENCEARRAYTOK]) and
...
//	writeln(Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].PassMethod,',',Tok[i + 2].Kind);
                  ...
                  if (Ident[IdentIndex].PassMethod = VARPASSING) and
		     ...
		     (Ident[IdentIndex].AllocElementType in Pointers) and
                     (Ident[IdentIndex].idType = DATAORIGINOFFSET) then
                  begin
                    ...
                    begin
                      ...
                      if (Ident[IdentIndex].DataType in Pointers) {and (Tok[i + 2].Kind = DEREFERENCETOK)} then
                        if (Ident[IdentIndex].AllocElementType = RECORDTOK) and (Tok[i + 3].Kind = DOTTOK) then
...
//	writeln('^',',', Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,' / ',Ident[IdentIndex].NumAllocElements_,' = ',Ident[IdentIndex].idType,',',Ident[IdentIndex].PassMethod,',',DEREFERENCE);
			     ...
			     if Ident[IdentIndex].idType <> DEREFERENCEARRAYTOK then
...

{
  ...
  i := CompileAddress(i + 1, ActualParamType, AllocElementType);
        ...
        if (ActualParamType in [POINTERTOK, STRINGPOINTERTOK]) and (Tok[i].Kind = IDENTTOK) then
        begin
          ...
          begin
            ...
            NumAllocElements_ := Ident[IdentTemp].NumAllocElements_;
          end;


          if Ident[IdentTemp].Kind in [PROCEDURETOK, FUNCTIONTOK] then
          begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Ident[ProcVarIndex].PassMethod = VARPASSING) then
          begin
    ...
    begin
      ...
      if (Tok[i + 2].Kind = CPARTOK) then
        ...
        begin
          ...
          if (Ident[IdentIndex].Param[NumActualParams].PassMethod = VARPASSING) then
          begin
            ...
            if (Ident[IdentIndex].Param[NumActualParams].DataType <> UNTYPETOK) and (ActualParamType = POINTERTOK) and (AllocElementType in [POINTERTOK, STRINGPOINTERTOK, PCHARTOK]) then
            begin
            ...
            begin
              ...
              if Ident[IdentTemp].Kind = FUNCTIONTOK then Error(i, CantAdrConstantExp);
              ...
              if Ident[IdentTemp].DataType in Pointers + [DEREFERENCEARRAYTOK]  then
                if not (Ident[IdentIndex].Param[NumActualParams].DataType in FileTypes) then
                begin

{
 ...
 writeln(Ident[IdentIndex].Param[NumActualParams].PassMethod,',', Ident[IdentTemp].PassMethod);
              ...
              if (Ident[IdentTemp].DataType in [RECORDTOK, OBJECTTOK])
              {and (Ident[IdentIndex].Param[NumActualParams].DataType in [RECORDTOK, OBJECTTOK])} then
              ...
              begin
                ...
                if (Ident[IdentTemp].AllocElementType = UNTYPETOK) then
                  if (Ident[IdentIndex].Param[NumActualParams].DataType <> UNTYPETOK) and
                ...
                if Ident[IdentIndex].Param[NumActualParams].DataType in Pointers + [DEREFERENCEARRAYTOK]  then
                begin
...

{
 ...
 writeln(Ident[IdentIndex].Param[NumActualParams].PassMethod,',', Ident[IdentTemp].PassMethod);
                      ...
                      if (Ident[IdentIndex].Param[NumActualParams].AllocElementType = UNTYPETOK) and
                        (Ident[IdentIndex].Param[NumActualParams].DataType in [POINTERTOK, PCHARTOK]) then
                      begin
...
//              writeln(Ident[IdentIndex].name,',', Ident[IdentIndex].kind,',',Ident[IdentIndex].Param[NumActualParams].DataType,',',Ident[IdentIndex].Param[NumActualParams].NumAllocElements,',',Ident[IdentIndex].Param[NumActualParams].AllocElementType ,'|',ActualParamType);
            ...
            if (ActualParamType in IntegerTypes) and (Ident[IdentIndex].Param[NumActualParams].DataType in RealTypes) then
            begin
            ...
            if (Ident[IdentIndex].Param[NumActualParams].DataType in [POINTERTOK]) then
              GetCommonType(i, Ident[IdentIndex].Param[NumActualParams].DataType, ActualParamType);


            if (Tok[i].Kind = IDENTTOK) and (Ident[IdentIndex].Param[NumActualParams].DataType = ENUMTOK) then
            begin
              IdentTemp := GetIdent(Tok[i].Name^);
            ...
            if (Tok[i].Kind = IDENTTOK) and (ActualParamType in [RECORDTOK, OBJECTTOK]) and not (Ident[IdentIndex].Param[NumActualParams].DataType in Pointers) then
              if Ident[GetIdent(Tok[i].Name^)].isNestedFunction then
              begin
...
//		writeln('>',Ident[IdentTemp].name,',', Ident[IdentTemp].AllocElementType,',', Ident[IdentTemp].NumAllocElements,'|',AllocElementType,',',Ident[IdentIndex].Param[NumActualParams].AllocElementType);

		  if Ident[IdentIndex].Param[NumActualParams].AllocElementType <> UNTYPETOK then
		  //if Ident[IdentTemp].DataType = POINTERTOK then
		    ...
		    AllocElementType := Ident[IdentTemp].AllocElementType
                ...
                if (Ident[IdentTemp].DataType in [RECORDTOK, OBJECTTOK]) then
                  ...
                  begin
                    ...
                    if (Ident[IdentIndex].Param[NumActualParams].AllocElementType = UNTYPETOK) and
                       (Ident[IdentIndex].Param[NumActualParams].DataType = POINTERTOK) and
		       (not ((Ident[IdentTemp].DataType = POINTERTOK) and (Ident[IdentTemp].AllocElementType in [RECORDTOK, OBJECTTOK]))) and
                       (Ident[IdentTemp].NumAllocElements > 1)
		    then
                      Error(i, IncompatibleTypesArray, IdentTemp, POINTERTOK)

                    else
                      if (Ident[IdentIndex].Param[NumActualParams].AllocElementType <> PROCVARTOK) and
		         (Ident[IdentIndex].Param[NumActualParams].DataType <> Ident[IdentTemp].DataType) and
                         (Ident[IdentIndex].Param[NumActualParams].NumAllocElements > 0) then

			if (Ident[IdentIndex].Param[NumActualParams].DataType in [POINTERTOK, DEREFERENCEARRAYTOK]) and
			   (Ident[IdentTemp].DataType in [POINTERTOK, DEREFERENCEARRAYTOK]) then
                ...
                if (Ident[IdentIndex].Param[NumActualParams].DataType in [POINTERTOK, STRINGPOINTERTOK]) and (Tok[i].Kind = IDENTTOK) then
                begin
                  IdentTemp := GetIdent(Tok[i].Name^);
                  ...
                  if (Ident[IdentTemp].DataType = STRINGPOINTERTOK) and (Ident[IdentTemp].NumAllocElements <> 0) and
                     (Ident[IdentIndex].Param[NumActualParams].DataType = POINTERTOK) and
                     (Ident[IdentIndex].Param[NumActualParams].NumAllocElements = 0) then
                    if Ident[IdentIndex].Param[NumActualParams].AllocElementType = UNTYPETOK then
                      Error(i, IncompatibleTypes, 0, Ident[IdentTemp].DataType, Ident[IdentIndex].Param[NumActualParams].DataType)
                    else
                      if Ident[IdentIndex].Param[NumActualParams].AllocElementType <> BYTETOK then
                ...
                begin
                  ...
                  if (Ident[IdentIndex].Param[NumActualParams].DataType = STRINGPOINTERTOK) then
                  begin    // CHAR -> STRING

                    if (ActualParamType = CHARTOK) and (Tok[i].Kind = CHARLITERALTOK) then
                    begin
                  ...
                  if (Ident[IdentIndex].Param[NumActualParams].DataType = PCHARTOK) then
                  begin
          ...
          if (Ident[IdentIndex].isRecursion = False) and (Ident[IdentIndex].isStdCall = False) and
            (ParamIndex > 1) and (Ident[IdentIndex].Param[NumActualParams].PassMethod <> VARPASSING) and
            ...
            begin
              ...
              if not (Ident[IdentIndex].Param[NumActualParams].AllocElementType in [RECORDTOK, OBJECTTOK]) then
              begin
    ...
    //   Error(i, UnresolvedForward, IdentIndex);
 ...
 if (Ident[IdentIndex].isExternal) and (Ident[IdentIndex].Libraries > 0) then begin
  ...
  if Ident[IdentIndex].isOverload then
   ...
   svar := GetLocalName(IdentIndex) + '.' + Ident[IdentIndex].Alias;
    ...
    if RCLIBRARY and Ident[IdentIndex].isExternal and (Ident[IdentIndex].Libraries > 0) and (Ident[IdentIndex].isStdCall = False) then
    begin
    ...
    begin
      ...
      if (Ident[IdentIndex].Block > 1) and (Ident[IdentIndex].Block <> BlockStack[BlockStackTop]) then
        ...
        for IdentTemp := NumIdent downto 1 do
          if (Ident[IdentTemp].Kind in [PROCEDURETOK, FUNCTIONTOK]) and (Ident[IdentTemp].ProcAsBlock = Ident[IdentIndex].Block) then
          begin
...

{
  if Ident[IdentIndex].UnitIndex > 1 then
   ...
   asm65(#9'.LOCAL +MAIN.' + svar);                  // w tym samym module poza aktualnym blokiem procedury/funkcji
    ...
    if (Ident[IdentIndex].Kind = FUNCTIONTOK) and (Ident[IdentIndex].isStdCall = False) and (Ident[IdentIndex].isRecursion = False) then
    begin
  ...
  IdentIndex := GetIdent(Tok[j].Name^)
   ...
   Error(i, TypeMismatch);
      ...
      begin
        ...
        if Tok[i + 2].Kind in OrdinalTypes + RealTypes + [POINTERTOK] then
        begin
      ...
      begin
        ...
        if Tok[i + 2].Kind = CHARLITERALTOK then
        begin
...
//	writeln(Ident[IdentIndex].name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].AllocElementType,',',Tok[i+3].Kind );
              ...
              begin
                ...
                begin								// length(CHAR) = 1
                  ...
                  begin
                    ...
                    if //((Ident[IdentIndex].DataType = STRINGPOINTERTOK) and (Ident[IdentIndex].NumAllocElements > 0)) or
                       ((Ident[IdentIndex].DataType in Pointers) and (Ident[IdentIndex].NumAllocElements > 0)) or
                       ((Ident[IdentIndex].DataType in Pointers) and (Ident[IdentIndex].NumAllocElements = 0) and (Tok[i + 3].Kind = DEREFERENCETOK)) then
                    begin

		      if Tok[i + 3].Kind = DEREFERENCETOK then inc(i);

                      if ((Ident[IdentIndex].DataType = STRINGPOINTERTOK) or (Ident[IdentIndex].AllocElementType = CHARTOK)) or
                        ((Ident[IdentIndex].DataType = POINTERTOK) and (Ident[IdentIndex].AllocElementType = STRINGPOINTERTOK)) then
                      begin

                        if Ident[IdentIndex].AllocElementType = STRINGPOINTERTOK then
                        begin    // length(array[x])
                          ...
                          if (Ident[IdentIndex].NumAllocElements * 2 > 256) or
                            (Ident[IdentIndex].NumAllocElements in [0, 1]) or (Ident[IdentIndex].PassMethod = VARPASSING) then
                          begin
  ...
  if Tok[i].Kind = ENDTOK then ExitLoop := TRUE;
    ...
    GenerateCaseEpilog(NumCaseStatements, CaseLocalCnt);
      ...
      begin
        ...
        if IdentIndex > 0 then
          if (Ident[IdentIndex].Kind = USERTYPE) and (Tok[i + 1].Kind = OPARTOK) then
          begin

            //    CheckTok(i + 1, OPARTOK);

            if (Ident[IdentIndex].DataType = POINTERTOK) and (Elements(IdentIndex) > 0) then
            begin
...
//	writeln(Ident[IdentIndex].name,',', DataSize[Ident[IdentIndex].AllocElementType],',', Ident[IdentIndex].AllocElementType ,',',ValType,',',VarType);
            ...
            if (ValType = POINTERTOK) and not (Ident[IdentIndex].DataType in [POINTERTOK, RECORDTOK, OBJECTTOK]) then
            begin
            ...
            if (ValType = POINTERTOK) and (Ident[IdentIndex].AllocElementType = PROCVARTOK) then
            begin

              IdentTemp := GetIdent('@FN' + IntToHex(Ident[IdentIndex].NumAllocElements_, 4));

              if Ident[IdentTemp].IsNestedFunction = False then
                Error(j, VarConstFuncExpected, IdentIndex);
	         ...
	         ((ValType = POINTERTOK) and (Ident[IdentIndex].DataType in [RECORDTOK, OBJECTTOK])) then
              begin
                ...
                if (Ident[IdentIndex].AllocElementType in [RECORDTOK, OBJECTTOK]) or (Ident[IdentIndex].DataType in [RECORDTOK, OBJECTTOK]) then
                begin
                  ...
                  if Tok[j + 2].Kind <> DOTTOK then yes := False
                  else

                    if Tok[j + 2].Kind = DOTTOK then
                    begin						// (pointer).field :=
            ...
            if (Ident[IdentIndex].DataType = POINTERTOK) and (Ident[IdentIndex].AllocElementType = PROCVARTOK) then
            begin
              ...
              //  if Ident[IdentTemp].IsNestedFunction = FALSE then
              //   Error(i, VarConstFuncExpected, IdentIndex));
              ...
              if Ident[IdentIndex].Kind = PROCEDURETOK then
                ...
                begin
                  ...
                  if (Tok[i + 1].Kind = DEREFERENCETOK) then
                    if (Ident[IdentIndex].Kind <> VARIABLE) or not (Ident[IdentIndex].DataType in Pointers + [DEREFERENCEARRAYTOK] ) then
...
//	writeln(Ident[IdentIndex].name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,',', Tok[i + 2].Kind);
                      ...
                      if (Ident[IdentIndex].DataType in [POINTERTOK, DEREFERENCEARRAYTOK]) and (Tok[i + 2].Kind = OBRACKETTOK) then
                      begin

                        i := CompileArrayIndex(i + 1, IdentIndex, ValType);              // array^[ ]

                        Push(0, ASPOINTERTOARRAYORIGIN2, DataSize[ValType], IdentIndex);

                      end
		      else
                      if (ValType = UNTYPETOK) and (Ident[IdentIndex].DataType = POINTERTOK) then
                      begin
                        ...
                        begin            // record^.
                          ...
                          begin
                            ...
                            if (Tok[i + 1].Kind = IDENTTOK) and (Tok[i + 2].Kind = OBRACKETTOK) then
                            begin    // record^.label[x]
                              ...
                              if ValType = STRINGPOINTERTOK then
                                Push(Ident[IdentIndex].Value, ASPOINTERTORECORD, DataSize[ValType], IdentIndex, IdentTemp and $ffff)
                              // record^.string
                              else
                                Push(Ident[IdentIndex].Value, ASPOINTERTOPOINTER, DataSize[ValType], IdentIndex, IdentTemp and $ffff);
                            // record_lebel.field^
                  ...
                  // -----------------------------------------------------------------------------
                    ...
                    if Tok[i + 1].Kind = OBRACKETTOK then					// Array element access
                      if not (Ident[IdentIndex].DataType in Pointers + [DEREFERENCEARRAYTOK])
                      {or ((Ident[IdentIndex].NumAllocElements = 0) and (Ident[IdentIndex].idType <> PCHARTOK))} then
                        // PByte, PWord
                        Error(i, IncompatibleTypeOf, IdentIndex)
                      else
                      begin
...
// perl
//	writeln('> ',Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[GetIdent(Tok[i].Name^)].name,',',VarType);

                          i := CompileArrayIndex(i, IdentIndex, ValType);		// array[ ]

			  IdentTemp := 0;


                          if (Tok[i + 2].Kind = DEREFERENCETOK) and (ValType in [RECORDTOK, OBJECTTOK])  then
                          begin								// array[ ]^
...
//	writeln(ValType,',',Ident[IdentIndex].Name + '||' + Tok[i].Name^,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].NumAllocElements_ );
                              ...
                              if (Tok[i + 2].Kind = OBRACKETTOK) and (ValType = STRINGPOINTERTOK) then
                              begin
...
//	writeln(Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].NumAllocElements_,',',Ident[IdentIndex].idType,'/',Ident[IdentIndex].Kind,' = ',Ident[IdentIndex].PassMethod ,' | ',ValType,',',Tok[j].Kind,',',Tok[j+1].kind];
                        ...
                        if (Ident[IdentIndex].PassMethod = VARPASSING) and
                          ...
                          (Ident[IdentIndex].AllocElementType in Pointers) and (Ident[IdentIndex].idType = DATAORIGINOFFSET) then
     ...
     Push(ConstVal, ASVALUE, DataSize[ValType], IdentIndex)
      ...
      begin
        ...
        begin
          ...
          if VarType = HALFSINGLETOK then ConstVal := CardToHalf(ftmp[1]) else
            if VarType = SINGLETOK then ConstVal := ftmp[1];
      ...
      begin
        ...
        if Tok[i + 1].Kind <> OPARTOK then
          Error(i, TypeIdentifierNotAllowed);
      ...
      begin
        ...
        if (ValType in Pointers) and (Tok[i + 2].Kind = IDENTTOK) and (Tok[i + 3].Kind <> OBRACKETTOK) then
        begin
  ...
  begin
    ...
    begin
      ...
      begin        // dla PLUSTOK, MINUSTOK rozszerz typ wyniku
        ...
        if (Tok[j + 1].Kind = MINUSTOK) and (RightValType in UnsignedOrdinalTypes) and (VarType in SignedOrdinalTypes + [BOOLEANTOK, REALTOK, HALFSINGLETOK, SINGLETOK]) then
        begin
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if (Tok[i].Kind = STRINGLITERALTOK) or (ValType = STRINGPOINTERTOK) then sLeft := Wordbool(1)
    else
      if (ValType in Pointers) and (Tok[i].Kind = IDENTTOK) then
        if (Ident[GetIdent(Tok[i].Name^)].AllocElementType = CHARTOK) and (Elements(GetIdent(Tok[i].Name^)) in [1..255]) then
          sLeft := Wordbool(1 or Elements(GetIdent(Tok[i].Name^)) shl 8);
    ...
    begin
      ...
      if SafeCompileConstExpression(k, ConstVal, ConstValType, VarType, False) then
        ...
        begin
          ...
          begin
            ...
            if (ValType in SignedOrdinalTypes) and (Tok[i + 1].Kind in [EQTOK, NETOK]) then
            begin
      ...
      if (Tok[i + 2].Kind = STRINGLITERALTOK) or (RightValType = STRINGPOINTERTOK) then sRight := Wordbool(1)
      else
        if (RightValType in Pointers) and (Tok[i + 2].Kind = IDENTTOK) then
          if (Ident[GetIdent(Tok[i + 2].Name^)].AllocElementType = CHARTOK) and (Elements(GetIdent(Tok[i + 2].Name^)) in [1..255]) then
            sRight := Wordbool(1 or Elements(GetIdent(Tok[i + 2].Name^)) shl 8);
      ...
      if cRight and (Tok[i + 1].Kind in [LTTOK, GTTOK]) and (ValType in IntegerTypes) then
      begin

        yes := False;

        if Tok[i + 1].Kind = LTTOK then
        begin
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          if (Ident[idx].Kind = CONSTTOK) then
          begin
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    for i := 0 to High(tb) - 1 do
      if ((lab.left >= tb[i].left) and (lab.left <= tb[i].right)) or ((lab.right >= tb[i].left) and (lab.right <= tb[i].right)) or ((tb[i].left >= lab.left) and (tb[i].right <= lab.right)) then
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if Ident[IdentIndex].PassMethod = CONSTPASSING then
      Error(i, CantAssignValuesToConst);
      ...
      begin
        ...
        if (IdentIndex > 0) and (Ident[IdentIndex].Kind = FUNCTIONTOK) and (BlockStackTop > 1) and (Tok[i + 1].Kind <> OPARTOK) then
          for j := NumIdent downto 1 do
            if (Ident[j].ProcAsBlock = NumBlocks) and (Ident[j].Kind = FUNCTIONTOK) then
            begin
        ...
        if IdentIndex > 0 then
	      ...
	      DEREFERENCE := false;
              ...
              if (Ident[IdentIndex].DataType = POINTERTOK) and (Ident[IdentIndex].AllocElementType = PROCVARTOK) and (not (Tok[i + 1].Kind in [ASSIGNTOK, OBRACKETTOK])) then
              begin
              ...
              if Ident[IdentIndex].IdType = DATAORIGINOFFSET then
              begin
              ...
              if (Ident[IdentIndex].Kind = TYPETOK) and (Tok[i + 1].Kind <> OPARTOK) then
                Error(i + 1, VariableExpected);


              if (Tok[i + 1].Kind = OPARTOK) and (Ident[IdentIndex].DataType = POINTERTOK) and (Elements(IdentIndex) > 0) then
              begin
...
// perl
                ...
                begin        							// (pointer)
                  ...
                  if (VarType in [RECORDTOK, OBJECTTOK]) and (Tok[i + 2].Kind = DOTTOK) then
                  begin
                    ...
                    if Tok[i + 2].Kind = DEREFERENCETOK then
                    begin
		    ...
		    DEREFERENCE := true;
                    ...
                    if not (Ident[IdentIndex].DataType in Pointers + [DEREFERENCEARRAYTOK]) then
                      Error(i + 1, IncompatibleTypeOf, IdentIndex);

                    IndirectionLevel := ASPOINTERTOPOINTER;

                    if (Ident[IdentIndex].DataType = STRINGPOINTERTOK) and (Ident[IdentIndex].NumAllocElements = 0) then begin
		      ...
		      if Ident[IdentIndex].AllocElementType in [RECORDTOK, OBJECTTOK] then VarType:=POINTERTOK;
                      ...
                      begin
                        ...
                        begin              					// pp^.field :=
                          ...
                          if GetIdent(Ident[IdentIndex].Name + '.' + Tok[i + 3].Name^) > 0 then
                            IdentIndex := GetIdent(Ident[IdentIndex].Name + '.' + Tok[i + 3].Name^);
...
// perl
                      if (Tok[i + 2].Kind = DEREFERENCETOK) and (VarType in [RECORDTOK, OBJECTTOK]) then
                      begin
                        Inc(i);

			if Tok[i + 2].Kind <> DOTTOK then
			  Push(0, ASPOINTERTORECORDARRAYORIGIN, DataSize[VarType], IdentIndex, 0);
//			 else
...
//	writeln(Ident[IdentIndex].Name,',',vartype,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].Kind,',',Tok[i + 2].Kind);
...
//-------
              ...
              if (Ident[IdentIndex].DataType = POINTERTOK) and (Ident[IdentIndex].AllocElementType = PROCVARTOK) and (Tok[i + 1].Kind <> ASSIGNTOK) then
              begin
...
//	writeln(Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].AllocElementType,',',IndirectionLevel,',',Tok[i + 2].Kind );


              if (Ident[IdentIndex].DataType = PCHARTOK) and   //( (IndirectionLevel in [ASPOINTER, ASPOINTERTOPOINTER]) or ((IndirectionLevel = ASPOINTERTOARRAYORIGIN) and (Ident[IdentIndex].PassMethod = VARPASSING)) ) and
                (IndirectionLevel = ASPOINTER) and (Tok[i + 2].Kind in [STRINGLITERALTOK, CHARLITERALTOK, IDENTTOK]) then
              begin
...
{$i include/compile_pchar.inc}
                ...
                if (Ident[IdentIndex].DataType in Pointers) and (Ident[IdentIndex].AllocElementType = CHARTOK) and
                  (Ident[IdentIndex].NumAllocElements > 0) and
                  ((IndirectionLevel in [ASPOINTER, ASPOINTERTOPOINTER]) or
                  ((IndirectionLevel = ASPOINTERTOARRAYORIGIN) and (Ident[IdentIndex].PassMethod = VARPASSING))) and
...
{$i include/compile_string.inc}
                ...
                begin					// Usual assignment
                  ...
                  if VarType = UNTYPETOK then
                    Error(i, AssignmentsNotPossible);
                  ...
                  if (VarType in [SHORTREALTOK, REALTOK]) and (ExpressionType in [SHORTREALTOK, REALTOK]) then ExpressionType := VarType;


                  if (VarType = POINTERTOK) and (ExpressionType = STRINGPOINTERTOK) then
                  begin

                    if (Ident[IdentIndex].AllocElementType = CHARTOK) then
                    begin							// +1
                      asm65(#9'lda :STACKORIGIN,x');
                      asm65(#9'add #$01');
                      asm65(#9'sta :STACKORIGIN,x');
                  ...
                  if (Tok[i].Kind = DEREFERENCETOK) and (VarType = POINTERTOK) and (ExpressionType = RECORDTOK) then
                  begin
...
//	  writeln(Ident[IdentIndex].Name,',', VarType,',', ExpressionType,' - ', Ident[IdentIndex].DataType,':',Ident[IdentIndex].AllocElementType,':',Ident[IdentIndex].NumAllocElements,' / ',IndirectionLevel);
                  ...
                  if VarType <> ExpressionType then
                    if (ExpressionType = POINTERTOK) and (Tok[k].Kind = IDENTTOK) then
                      if (Ident[GetIdent(Tok[k].Name^)].DataType = POINTERTOK) and (Ident[GetIdent(Tok[k].Name^)].AllocElementType = PROCVARTOK) then
                      begin
                  ...
                  begin
                    ...
                    if (Ident[IdentIndex].DataType in [RECORDTOK, OBJECTTOK] + Pointers) then
                    begin

                      if (ExpressionType in Pointers - [STRINGPOINTERTOK]) and (Tok[k].Kind = IDENTTOK) then
                      begin
         ...
         if (Elements(IdentTemp) > 0) and (Tok[i + 3].Kind <> OBRACKETTOK) then
          Error(k, IncompatibleTypesArray, IdentTemp, ExpressionType )
                        ...
                        // GetCommonType(i + 1, VarType, RECORDTOK)
                          ...
                          if (Ident[IdentIndex].AllocElementType <> UNTYPETOK) and
                             (Ident[IdentTemp].AllocElementType <> UNTYPETOK) and
			     (Ident[IdentTemp].AllocElementType <> Ident[IdentIndex].AllocElementType) and (Tok[k + 1].Kind <> OBRACKETTOK) then
                          begin

                            if ((Ident[IdentTemp].NumAllocElements > 0) {and (Ident[IdentTemp].AllocElementType <> RECORDTOK)}) and
                              ((Ident[IdentIndex].NumAllocElements > 0) {and (Ident[IdentIndex].AllocElementType <> RECORDTOK)}) then
...
//	writeln('1> ',Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,'/',Ident[IdentIndex].NumAllocElements_,', P:', Ident[IdentIndex].PassMethod,' | ',VarType,',',ExpressionType,',',IndirectionLevel);
                      ...
                      if (VarType = ENUMTYPE) {and (Tok[k].Kind = IDENTTOK)} then
                      begin
                        ...
                        if (IdentTemp > 0) and (Ident[IdentTemp].Kind = USERTYPE) and (Ident[IdentTemp].DataType = ENUMTYPE) then
                        begin
                          ...
                          begin
                            ...
                            if (Ident[IdentIndex].DataType = ENUMTOK) and (ExpressionType in UnsignedOrdinalTypes + [ENUMTOK]) then
                      ...
                      begin
                        ...
                        if (Tok[k].Kind = IDENTTOK) and (Tok[k + 1].Kind = SEMICOLONTOK) then
                          IdentTemp := GetIdent(Tok[k].Name^)
                        else
                          IdentTemp := 0;
...
//	writeln('0> ',Ident[IdentIndex].Name,',',VarType,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements,' | ', ExpressionType,',',IndirectionLevel);
		     ...
		     (Ident[IdentIndex].DataType = POINTERTOK) and
                     (Ident[IdentIndex].NumAllocElements = 0) and (ExpressionType <> POINTERTOK) then
                  begin
                  ...
                  if (VarType in [RECORDTOK, OBJECTTOK]) or ((VarType = POINTERTOK) and (ExpressionType in [RECORDTOK, OBJECTTOK])) then
                  begin
			    ...
			    if (Ident[IdentTemp].NumAllocElements_ > 0) and (Tok[Result].Kind = DEREFERENCETOK) and
			       (Ident[IdentIndex].DataType = RECORDTOK) and
			       ((Ident[IdentTemp].DataType = POINTERTOK) and (Ident[IdentTemp].AllocElementType = RECORDTOK)) and
			       (Ident[IdentTemp].NumAllocElements = Ident[IdentIndex].NumAllocElements)
			    then
                    ...
                    if (Ident[IdentIndex].DataType = POINTERTOK) and (Ident[IdentIndex].AllocElementType in [RECORDTOK, OBJECTTOK]) and (Ident[IdentIndex].PassMethod = Ident[IdentTemp].PassMethod) then
                    begin
                      ...
                      if yes and (ADDRESS = False) and (ExpressionType in [RECORDTOK, OBJECTTOK]) then
                        if (Ident[IdentTemp].DataType = POINTERTOK) and
                          (Ident[IdentTemp].AllocElementType in [RECORDTOK, OBJECTTOK]) then
		       ...
		       ((ExpressionType = POINTERTOK) and (Ident[IdentTemp].AllocElementType in [RECORDTOK, OBJECTTOK])) then
                    begin
                      ...
                      DEREFERENCE := False;
                      if (Tok[k + 1].Kind = DEREFERENCETOK) then
                      begin
                        Inc(k);
                      ...
                      a65(__subBX);
{
                      ...
                      if (Ident[IdentIndex].DataType = RECORDTOK) and (Ident[IdentTemp].DataType = RECORDTOK) and (Ident[IdentTemp].AllocElementTYpe = RECORDTOK) then
                      begin
...
//	writeln('--- ', IndirectionLevel);
			    ...
			    if IndirectionLevel in [ASPOINTERTOARRAYORIGIN, ASPOINTERTOARRAYORIGIN2] then begin
			      ...
			      if (Ident[IdentIndex].AllocElementType = RECORDTOK) and (Ident[IdentIndex].NumAllocElements_ > 0) and
			         (Ident[IdentTemp].AllocElementType = RECORDTOK) and (Ident[IdentTemp].NumAllocElements_ = 0) then
			      begin
				  ...
				  a65(__subBX);
                            ...
                            if (Ident[IdentIndex].DataType = RECORDTOK) and (Ident[IdentTemp].DataType = POINTERTOK) then
                            begin

//	writeln(Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType ,',',Ident[IdentIndex].NumAllocElements,'/',Ident[IdentIndex].NumAllocElements_,',',Ident[IdentIndex].pASSmETHOD);
//	writeln(Ident[IdentTemp].Name,',',Ident[IdentTemp].DataType,',',Ident[IdentTemp].AllocElementType ,',',Ident[IdentTemp].NumAllocElements,'/',Ident[IdentTemp].NumAllocElements_,',',Ident[IdentTemp].pASSmETHOD);
//	writeln('--- ', IndirectionLevel,',',DEREFERENCE);
			        ...
			        if (Ident[IdentTemp].AllocElementType = RECORDTOK) and (Ident[IdentTemp].NumAllocElements_ > 0) then begin
				  ...
				  asm65(#9'lda :STACKORIGIN+1,x');
			    	  ...
			    	  asm65(#9'sta :TMP+1');
                                  ...
                                  begin
                                    ...
                                    asm65(#9'@move ":TMP" ' + Name + GetLocalName(IdentIndex, 'adr.') + ' #' + IntToStr(RecordSize(IdentIndex)));
                    ...
                    {and (Ident[IdentIndex].AllocElementType in [RECORDTOK, OBJECTTOK])} then
                    begin

//	writeln(Ident[IdentIndex].Name,',',Ident[IdentIndex].DataType,',',Ident[IdentIndex].NumAllocElements,',',Ident[IdentIndex].AllocElementType ,' / ', VarType,',',ExpressionType,',',IndirectionLevel,',',Tok[k].Kind,',',DEREFERENCE);


		      if (Tok[k].Kind <> ADDRESSTOK) and (IndirectionLevel in [ASPOINTER, ASPOINTERTOPOINTER]) then begin

		       if (DEREFERENCE = false) and (Ident[IdentIndex].NumAllocElements = 0) and (Tok[k].Kind = STRINGLITERALTOK) then
		         Error(i, 'Incompatible types: got "' + InfoAboutToken(Tok[k].Kind) + '" expected "^' + Ident[IdentIndex].Name + '"');

		       if (DEREFERENCE = false) and (Ident[IdentIndex].NumAllocElements = 0) and (Tok[k].Kind = IDENTTOK) and (Ident[GetIdent(Tok[k].Name^)].NumAllocElements > 0) then
		         Error(i, 'Incompatible types: got "' + InfoAboutToken(Ident[GetIdent(Tok[k].Name^)].DataType) + '" expected "' + Ident[IdentIndex].Name + '"');

		       if DEREFERENCE and (Ident[IdentIndex].NumAllocElements = 0) and (Tok[k].Kind = IDENTTOK) and (Tok[k + 1].Kind <> DEREFERENCETOK) and (Ident[GetIdent(Tok[k].Name^)].NumAllocElements = 0) then
		         Error(i, 'Incompatible types: got "^' + InfoAboutToken(Ident[GetIdent(Tok[k].Name^)].DataType) + '" expected "' + InfoAboutToken(Ident[GetIdent(Tok[k].Name^)].DataType) + '"');

		      end;


                      if (Tok[k].Kind <> ADDRESSTOK) and (IndirectionLevel in [ASPOINTERTOARRAYORIGIN, ASPOINTERTOARRAYORIGIN2]) and (Ident[IdentIndex].AllocElementType = STRINGPOINTERTOK) then
                      begin

                        if (Tok[k].Kind = IDENTTOK) and (Ident[GetIdent(Tok[k].Name^)].AllocElementType <> UNTYPETOK) then
                    ...
                    // dla PROC, FUNC -> Ident[GetIdent(Tok[k].Name^)].NumAllocElements -> oznacza liczbe parametrow takiej procedury/funkcj]
...

{
        ...
        ResetOpty;
                            ...
                            if j <> Integer(Elements(IdentTemp) * DataSize[Ident[IdentTemp].AllocElementType]) then
                            begin

                              if (Ident[IdentIndex].NumAllocElements_ > 0) and ((Ident[IdentIndex].NumAllocElements_ = Ident[IdentTemp].NumAllocElements) or (Ident[IdentIndex].NumAllocElements_ = Ident[IdentTemp].NumAllocElements_)) then
                              begin

                                //writeln(Tok[k].line,',', Ident[IdentTemp].NumAllocElements_);

                                if Ident[IdentTemp].NumAllocElements_ = 0 then
                                begin
      ...
      begin
        ...
        j := CompileStatement(i + 1);
        while (Tok[j + 1].Kind = SEMICOLONTOK) or ((Tok[j + 1].Kind = COLONTOK) and (Tok[j].Kind = IDENTTOK)) do
	   ...
	   FUNCTIONTOK: begin
        ...
        repeat  // Loop over all cases
          ...
          repeat  // Loop over all constants for the current case
            ...
            if (Tok[i].Kind = IDENTTOK) then
              if ((EnumName = '') and (GetEnumName(GetIdent(Tok[i].Name^)) <> '')) or
                ((EnumName <> '') and (GetEnumName(GetIdent(Tok[i].Name^)) <> EnumName)) then begin

{
              IdentTemp := GetIdent(Tok[i].Name^);

	      writeln(SelectorType,',', Ident[GetIdent('GAME.MOVECHUJ')].Kind  );


              writeln( Tok[i].Name^,',', Ident[IdentTemp].Kind,',', Ident[IdentTemp].AllocElementType              );

	writeln(EnumName,',',GetEnumName(GetIdent(Tok[i].Name^)) );
}
                Error(i, 'Constant and CASE types do not match');
...
{$ENDIF}
      ...
      begin
        ...
        begin
          ...
          if IdentIndex > 0 then
            if not ((Ident[IdentIndex].Kind = VARIABLE) and (Ident[IdentIndex].DataType in OrdinalTypes + Pointers + [SUBRANGETYPE])) then
              ...
              begin
                ...
                if Tok[i + 2].Kind = INTOK then
                begin    // IN

                  j := i + 3;

                  if Tok[j].Kind = STRINGLITERALTOK then
                  begin
  ...
  {$ENDIF}
                    ...
                    begin
                      ...
                      if ((Down = False) and (forLoop.end_value >= forLoop.begin_value)) or (Down and (forLoop.end_value <= forLoop.begin_value)) then
                      begin
      ...
      ASSIGNFILETOK:
        if Tok[i + 1].Kind <> OPARTOK then
          ...
          begin
            ...
            if not ((Ident[IdentIndex].DataType in FileTypes) or (Ident[IdentIndex].AllocElementType in FileTypes)) then
              Error(i + 2, IncompatibleTypeOf, IdentIndex);
          ...
          begin
            ...
            begin
              if Ident[IdentIndex].NumAllocElements * DataSize[Ident[IdentIndex].AllocElementType] = 0 then
                ...
                Push(Integer(Ident[IdentIndex].NumAllocElements * DataSize[Ident[IdentIndex].AllocElementType]), ASVALUE, 2);
      ...
      READLNTOK:
        ...
        begin
          ...
          begin
            ...
            if (IdentIndex > 0) and (Ident[IdentIndex].DataType = TEXTFILETOK) then
            begin
              ...
              if IdentIndex > 0 then
                ...
                begin
                  ...
                  if (Ident[IdentIndex].DataType in Pointers) and (Ident[IdentIndex].NumAllocElements > 0) and (Ident[IdentIndex].AllocElementType = CHARTOK) then
                  begin     // string
      ...
      begin
        ...
        begin
          ...
          if (Tok[i + 1].Kind = IDENTTOK) and (Ident[GetIdent(Tok[i + 1].Name^)].DataType = TEXTFILETOK) then
          begin
            ...
            repeat
              ...
              case Tok[i + 1].Kind of
                ...
                begin
                  ...
                  if (ExpressionType = CHARTOK) and (Tok[i].Kind = DEREFERENCETOK) and (Tok[i - 1].Kind <> IDENTTOK) then
                  begin
...
//	writeln(Ident[IdentIndex].Name,',',ExpressionType,' | ',Ident[IdentIndex].DataType,',',Ident[IdentIndex].AllocElementType,',',Ident[IdentIndex].NumAllocElements_,',',Ident[IdentIndex].Kind);
      ...
      begin
        ...
        if isInterrupt and ((pos(' :bp', AsmBlock[AsmBlockIndex]) > 0) or (pos(' :STACK', AsmBlock[AsmBlockIndex]) > 0)) then
        begin
      ...
      begin
        ...
        begin          // first parameter
          ...
          begin
            ...
            if {((Ident[IdentIndex].DataType in Pointers) and (Ident[IdentIndex].NumAllocElements=0)) or}
            (Ident[IdentIndex].DataType = REALTOK) then
              Error(i, LeftNotAssigned)
            else
            begin
        ...
        if not (Ident[IdentIndex].idType in [PCHARTOK]) and (Ident[IdentIndex].DataType in Pointers) and (Ident[IdentIndex].NumAllocElements > 0) and
          (not (Ident[IdentIndex].AllocElementType in [RECORDTOK, OBJECTTOK])) then
        begin
	  ...
	  if (Tok[i + 1].Kind = DEREFERENCETOK) and (Tok[i + 2].Kind = OBRACKETTOK) and (Ident[IdentIndex].DataType in [POINTERTOK, DEREFERENCEARRAYTOK]) then inc(i);
        ...
        begin        // potencjalnie drugi parametr
          ...
          begin
            ...
            if (Ident[IdentIndex].DataType in Pointers) and (Ident[IdentIndex].AllocElementType in [RECORDTOK, OBJECTTOK]) then
            begin
        ...
        else  // if Tok[i + 1].Kind = COMMATOK

          if (Ident[IdentIndex].PassMethod = VARPASSING) or ((Ident[IdentIndex].DataType in Pointers) and
            (Ident[IdentIndex].AllocElementType in OrdinalTypes + Pointers + [RECORDTOK, OBJECTTOK])) then

            if (Ident[IdentIndex].PassMethod = VARPASSING) or (Ident[IdentIndex].NumAllocElements > 0) or
              (IndirectionLevel = ASPOINTERTOPOINTER) or ((Ident[IdentIndex].NumAllocElements = 0) and
            ...
            begin
              ...
              if not (Ident[IdentIndex].AllocElementType in [BYTETOK, SHORTINTTOK]) then
              begin
        ...
        if (Ident[IdentIndex].PassMethod = VARPASSING) and (IndirectionLevel <> ASPOINTERTOARRAYORIGIN) then
          IndirectionLevel := ASPOINTERTOPOINTER;
      ...
      begin
        ...
        begin
          ...
          for j := 1 to NumIdent do
            if (Ident[j].ProcAsBlock = BlockStack[BlockStackTop]) and (Ident[j].Kind = FUNCTIONTOK) then
            begin
...
// ----------------------------------------------------------------------------
  ...
  var
          ...
          begin
            ...
            begin
              ...
              if Ident[IdentIndex].Value < 0 then
                Result := #9'= DATAORIGIN+' + Hex(abs(Ident[IdentIndex].Value), 4)
              else
                if abs(Ident[IdentIndex].Value) < 256 then
                  Result := #9'= ' + Hex(Byte(Ident[IdentIndex].Value), 2)
                else
                  Result := #9'= ' + Hex(Ident[IdentIndex].Value, 4);

            end
            else

              if Ident[IdentIndex].NumAllocElements > 0 then
                Result := #9'= CODEORIGIN+' + Hex(Ident[IdentIndex].Value - CODEORIGIN_BASE - CODEORIGIN, 4)
              else
                if abs(v) < 256 then
                  Result := #9'= ' + Hex(Byte(v), 2)
                else
                  Result := #9'= ' + Hex(v, 4);
    ...
    begin
      ...
      if Ident[IdentIndex].AllocElementType in [BYTETOK..FORWARDTYPE] then
      begin
  ...
  begin
    ...
    begin
      ...
      for IdentIndex := 1 to NumIdent do
        if (Ident[IdentIndex].Block = Ident[BlockIdentIndex].ProcAsBlock) and (Ident[IdentIndex].UnitIndex = UnitNameIndex) then
        begin
            ...
            case Ident[IdentIndex].Kind of
              ...
              VARIABLE: if Ident[IdentIndex].isAbsolute then
                begin    // ABSOLUTE = TRUE
                  ...
                  if (Ident[IdentIndex].PassMethod <> VARPASSING) and
                    (Ident[IdentIndex].DataType in [RECORDTOK, OBJECTTOK] + Pointers) and
                    (Ident[IdentIndex].NumAllocElements > 0) then
                  begin
                  ...
                  begin
                    ...
                    if ((Ident[IdentIndex].IdType <> ARRAYTOK) and (Ident[IdentIndex].AllocElementType in [RECORDTOK, OBJECTTOK])) or
                       (Ident[IdentIndex].IdType = DATAORIGINOFFSET)
		    then
                    ...
                    begin
                      ...
                      if Ident[IdentIndex].idType <> DATAORIGINOFFSET then      // indeksy do RECORD nie zliczaj
                        ...
                        if (Ident[IdentIndex].Name = 'RESULT') and (Ident[BlockIdentIndex].Kind = FUNCTIONTOK) then

                        // zliczaj RESULT aby rekurencja zadzialala w przypadku kiedy RESULT nie jest na końcu bloku alokacji zmiennych
{
...
// ----------------------------------------------------------------------------
  ...
  var
    ...
    begin
      ...
      if (ConstValType in [SINGLETOK, HALFSINGLETOK]) and (ActualParamType = REALTOK) then ActualParamType := ConstValType;
      ...
      if (ConstValType = SHORTREALTOK) and (ActualParamType = REALTOK) then ActualParamType := SHORTREALTOK;
...
{
 if Ident[BlockIdentIndex].isOverload the]
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if (Tok[i + 3].Kind = CPARTOK) and (Tok[i + 2].Kind = OPARTOK) then
      ...
      begin
        ...
        repeat
          ...
          if (ListPassMethod in [CONSTPASSING, VARPASSING]) and (Tok[i].Kind <> COLONTOK) then
          begin
    ...
    while Tok[i + 1].Kind in [OVERLOADTOK, ASSEMBLERTOK, FORWARDTOK, REGISTERTOK, INTERRUPTTOK, PASCALTOK, STDCALLTOK, INLINETOK, KEEPTOK] do
    begin
...
//    Name: String;
  begin
    ...
    for TypeIndex := 1 to NumIdent do
      if (Ident[TypeIndex].AllocElementType = FORWARDTYPE) and (Ident[TypeIndex].Block = BlockStack[BlockStackTop]) then
      begin
  ...
  begin
    ...
    begin
      ...
      begin                      // array [0..x] of record
        ...
        _VarDataSize := tmpVarDataSize + (NumAllocElements shr 16) * DataSize[POINTERTOK];
          ...
          begin
            ...
            if isAbsolute then
              if not (Types[NumAllocElements].Field[ParamIndex].DataType in [RECORDTOK, OBJECTTOK]) then
                ...
                Inc(ConstVal, _VarDataSize - tmpVarDataSize_);
  ...
  begin
    ...
    begin
      ...
      for ParamIndex := 1 to Types[Ident[BlockIdentIndex].ObjectIndex].NumFields do
        if Types[Ident[BlockIdentIndex].ObjectIndex].Field[ParamIndex].ObjectVariable = False then
        begin
          ...
          if ParamIndex <> Types[Ident[BlockIdentIndex].ObjectIndex].NumFields then
          begin

            if (Types[Ident[BlockIdentIndex].ObjectIndex].Field[ParamIndex].DataType = POINTERTOK) and (Types[Ident[BlockIdentIndex].ObjectIndex].Field[ParamIndex].NumAllocElements > 0) then
            begin
    ...
    begin
      ...
      begin
        ...
        if (Param[ParamIndex].DataType = POINTERTOK) and (Param[ParamIndex].AllocElementType in [RECORDTOK, OBJECTTOK]) then
        begin    // fix issue #94
    ...
    if (Ident[BlockIdentIndex].ObjectIndex = 0) then
      ...
      begin
        ...
        if (yes = False) and (NumParams = 1) and (DataSize[Param[1].AllocElementType] = 1) and (Param[1].PassMethod <> VARPASSING) then
	  asm65(#9'sta ' + Param[1].Name);

      end
      else

        if (yes = False) and (NumParams = 1) and (DataSize[Param[1].DataType] = 1) and (Param[1].PassMethod <> VARPASSING) then
          asm65(#9'sta ' + Param[1].Name);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Param[ParamIndex].PassMethod <> VARPASSING) and (Param[ParamIndex].DataType in [RECORDTOK, OBJECTTOK] + Pointers) and (Param[ParamIndex].NumAllocElements and $FFFF > 1) then
            // copy arrays
...

{
  writeln(Types[Ident[BlockIdentIndex].ObjectIndex].Field[ParamIndex].Name,',',
        ...
        if Types[Ident[BlockIdentIndex].ObjectIndex].Field[ParamIndex].DataType = OBJECTTOK then
          Error(i, '-- under construction --');
        ...
        _VarDataSize := tmpVarDataSize + DataSize[POINTERTOK];

        if Types[Ident[BlockIdentIndex].ObjectIndex].Field[ParamIndex].ObjectVariable then
        begin
    ...
    while Tok[i].Kind in [CONSTTOK, TYPETOK, VARTOK, LABELTOK, PROCEDURETOK, FUNCTIONTOK,
    ...
    begin
      ...
      if Tok[i].Kind = LOOPUNROLLTOK then
      begin
      ...
      if Tok[i].Kind = NOLOOPUNROLLTOK then
      begin
      ...
      if Tok[i].Kind = PROCALIGNTOK then
      begin
      ...
      if Tok[i].Kind = LOOPALIGNTOK then
      begin
      ...
      if Tok[i].Kind = LINKALIGNTOK then
      begin
      ...
      if Tok[i].Kind = UNITBEGINTOK then
      begin
        asm65separator;
      ...
      if Tok[i].Kind = IMPLEMENTATIONTOK then
      begin
      ...
      if (Tok[i].Kind = INITIALIZATIONTOK) or ((PublicSection = False) and (Tok[i].Kind = BEGINTOK)) then
      begin

        if not ImplementationUse then CheckTok(i, IMPLEMENTATIONTOK);
      ...
      begin       // na samym poczatku listingu
        ...
        if (Tok[i + 2].Kind = COLONTOK) and (Tok[i + 3].Kind = INTNUMBERTOK) then
        begin
      ...
      begin       // na samym poczatku listingu
        ...
        if (Tok[i + 1].Kind = COLONTOK) and (Tok[i + 2].Kind = INTNUMBERTOK) then
        begin
      ...
      begin    // co najwyzej po PROGRAM
        ...
        begin
          ...
          if Tok[j].Kind <> LIBRARYTOK then CheckTok(i, BEGINTOK);
        ...
        begin
          ...
          if Tok[j].Kind <> PROGRAMTOK then CheckTok(i, BEGINTOK);

        end;

        if INTERFACETOK_USE then
          if Tok[i - 1].Kind <> INTERFACETOK then CheckTok(i, IMPLEMENTATIONTOK);

        if ImplementationUse then
          if Tok[i - 1].Kind <> IMPLEMENTATIONTOK then CheckTok(i, BEGINTOK);
        ...
        repeat
          ...
          if Tok[i + 1].Kind <> IDENTTOK then
            ...
            begin
              ...
              if Tok[i + 2].Kind = COLONTOK then
              begin

                open_array := False;


                if (Tok[i + 3].Kind = ARRAYTOK) and (Tok[i + 4].Kind = OFTOK) then
                begin
                  ...
                  begin
                    ...
                    if (Ident[NumIdent].NumAllocElements in [0, 1]) and (open_array = False) then
                      ...
                      begin                  // const array of type = [ ]
                        ...
                        if (Tok[j + 2].Kind = STRINGLITERALTOK) and (AllocElementType = CHARTOK) then
                        begin  // = 'string'
      ...
      begin
        repeat
          ...
          begin
            ...
            if (Tok[i + 3].Kind = ARRAYTOK) and (Tok[i + 4].Kind <> OBRACKETTOK) then
            begin
              j := CompileType(i + 5, VarType, NumAllocElements, AllocElementType);
      ...
      begin
        ...
        if (Tok[i + 1].Kind = OBRACKETTOK) and (Tok[i + 2].Kind in [VOLATILETOK, STRIPEDTOK]) then
        begin
        ...
        repeat
          ...
          if Tok[i + 1].Kind = PACKEDTOK then
          begin

            if (Tok[i + 2].Kind in [ARRAYTOK, RECORDTOK]) then
            begin
              Inc(i);
              // pack := true;
            end
          ...
          if (IdType = ARRAYTOK) and (Tok[i + 2].Kind = OFTOK) then
          begin      // array of type [Ordinal Types]

            i := CompileType(i + 3, VarType, NumAllocElements, AllocElementType);

            if VarType = SUBRANGETYPE then
  	    begin
	      VarType := Types[NumAllocElements].Field[0].AllocElementType;
	      NumAllocElements := 0;
	      AllocElementType := UNTYPETOK;
	    end;
	    ...
	    begin
          ...
          if Tok[i + 1].Kind = REGISTERTOK then
          begin
            ...
            if Tok[i + 1].Kind = EXTERNALTOK then
            begin
              ...
              if Tok[i + 1].Kind = ABSOLUTETOK then
              begin
                ...
                if (Tok[i + 1].Kind = IDENTTOK) and (Ident[GetIdent(Tok[i + 1].Name^)].Kind = VARTOK) then
                begin
          ...
          tmpVarDataSize := _VarDataSize;    // dla ABSOLUTE, RECORD
	      ...
	      if AllocElementType in [RECORDTOK, OBJECTTOK] then
            ...
            begin
              ...
              if isStriped and (Ident[NumIdent].PassMethod <> VARPASSING) then
              begin
...
//	writeln(Ident[NumIdent].Name,',',VarType, ' / ', AllocElementType ,' = ',NestedDataType, ',',NestedAllocElementType,',', hexStr(NestedNumAllocElements,8),',',hexStr(NumAllocElements,8));


{
if (VarType = POINTERTOK) and (AllocElementType = RECORDTOK) and (NumAllocElements shr 16 = 0) then begin

                SaveToDataSegment(Ident[NumIdent].Value - DATAORIGIN, _VarDataSize-2, DATAORIGINOFFSET);
end;
}

              if (VarType = POINTERTOK) and (AllocElementType = STRINGPOINTERTOK) and (NestedNumAllocElements > 0) and (NumAllocElements > 1) then
              begin  // array [ ][ ] of string;
                ...
                begin
                  ...
                  begin
                    SaveToDataSegment(idx, _VarDataSize, DATAORIGINOFFSET);

                    Inc(idx, 2);
                    inc(_VarDataSize, NestedNumAllocElements);
                  end;
          ...
          if isAbsolute and (open_array = False) then
            ...
            begin
              ...
              begin
                ...
                //   if Ident[NumIdent].NumAllocElements = 0 the]
                //    Error(i + 1, 'Illegal expression');

                Inc(i);


                if Tok[i].Kind = ADDRESSTOK then
                begin
                  ...
                  if Tok[i].Kind = CHARLITERALTOK then
                  begin
      ...
      SaveToDataSegment(idx, Tok[i].StrAddress - CODEORIGIN + 1, CODEORIGINOFFSET)
       ...
       if (IdType = ARRAYTOK) and (AllocElementType = CHARTOK) then begin      // var label: array of char = ''
        ...
        if Tok[i].StrLength > NumAllocElements then
         ...
         for j := 0 to Ident[NumIdent].NumAllocElements-1 d]
         ...
         for j := 0 to ParamIndex-1 do              // var label: string = ''
           SaveToDataSegment(idx + j, ord( StaticStringData[ Tok[i].StrAddress - CODEORIGIN + j ] ), BYTETOK);
                      ...
                      if (Ident[NumIdent].NumAllocElements in [0, 1]) and (open_array = False) then
                        ...
                        begin                   // array of type = [ ]
                          ...
                          if (Tok[i].Kind = STRINGLITERALTOK) and (AllocElementType = CHARTOK) then
                          begin    // = 'string'
                          ...
                          inc(_VarDataSize, NumAllocElements * DataSize[Ident[NumIdent].AllocElementType]);
          ...
          if (Tok[i + 2].Kind = OBRACKETTOK) and (Tok[i + 3].Kind in [VOLATILETOK, STRIPEDTOK]) then
          begin
        ...
        begin
          ...
          if ForwardIdentIndex <> 0 then
            if (Ident[ForwardIdentIndex].IsUnresolvedForward) and (Ident[ForwardIdentIndex].Block = BlockStack[BlockStackTop]) then
              if Tok[i].Kind <> Ident[ForwardIdentIndex].Kind then
                Error(i, UnresolvedForward, ForwardIdentIndex);
          ...
          begin
            ...
            begin
              ...
              if ((Pass = CODEGENERATIONPASS) and (not Ident[NumIdent].IsNotDead)) then
            ...
            begin
              ...
              if ((Pass = CODEGENERATIONPASS) and (not Ident[ForwardIdentIndex].IsNotDead)) then
                // Do not compile dead procedures and functions
              begin
                OutputDisabled := True;
              end;
...
//      GenerateAssignment(Ident[IdentIndex].Value, ASPOINTER, DataSize[POINTERTOK], IdentIndex);
    ...
    if Ident[BlockIdentIndex].Kind in [PROCEDURETOK, FUNCTIONTOK, CONSTRUCTORTOK, DESTRUCTORTOK] then
    begin
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if Tok[j].Kind = ENDTOK then CheckTok(j + 1, DOTTOK)
    else
...
if LIBRARY_USE = FALSE then begin
  ...
  if target.id = ___a8 then begin
    ...
    begin
      ...
      begin
        ...
        DataSegmentSize := _VarDataSize;

        if LIBRARYTOK_USE = False then
          for j := _VarDataSize - 1 downto 0 do
 ...
 asm65(#13#10#9'.print ''DATA: '',DATAORIGIN,''..'',DATAORIGIN+'+IntToStr(VarDataSize));
    ...
    begin
      ...
      asm65('VARDATASIZE'#9'= ' + IntToStr(_VarDataSize));
